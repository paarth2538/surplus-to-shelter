      ...
      {detailsPickup ? (
        <div className="driver-modal-backdrop" role="presentation" onClick={() => setDetailsPickup(null)}>
          <div className="driver-modal-window pickup-details-modal" role="dialog" aria-modal="true" aria-labelledby="pickup-details-title" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="pickup-detail-grid"><div><span>Donor</span><strong>{detailsPickup.donor?.name || 'Donor partner'}</strong>{detailsPickup.donor?.phone ? <a className="btn-call-link" style={{ marginTop: 4 }} href={`tel:${detailsPickup.donor.phone}`}>📞 Call Donor ({detailsPickup.donor.phone})</a> : <small>{detailsPickup.donor?.email || 'Contact details unavailable'}</small>}</div><div><span>Shelter</span><strong>{detailsPickup.shelters?.organization_name || 'Shelter destination pending'}</strong>{detailsPickup.shelters?.phone ? <a className="btn-call-link" style={{ marginTop: 4 }} href={`tel:${detailsPickup.shelters.phone}`}>📞 Call Shelter ({detailsPickup.shelters.phone})</a> : <small>Contact details unavailable</small>}</div><div><span>Cargo</span><strong>{detailsPickup.donations?.quantity || '—'} {detailsPickup.donations?.unit || 'units'}</strong><small>{detailsPickup.donations?.food_name || 'Surplus goods'}</small></div><div><span>Scheduled</span><strong>{formatDate(detailsPickup.pickup_time)}</strong><small>{detailsPickup.notes || 'No additional notes'}</small></div></div>
